         ...
         <Link href={s.link || '#'} className="flex flex-col justify-between h-full">
            <div>
               <div className="relative aspect-[16/12] bg-gradient-to-br from-brand via-brand-bright to-ocean overflow-hidden">
                  {s.image ? (
                     <Image src={`${process.env.NEXT_PUBLIC_APP_URL}${s.image}`} alt={s.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover transition-transform duration-500 group-hover:scale-105" />
                  ) : (
