...
const SCHEDULES = [
  {
    id: 1,
    title: "Viva Practical Exam Schedule B Sc Regular Semester 1 FAS Winter Examination 2025 26 08",
    url: "/examination/dummy-pdf.pdf",
  },
  {
    id: 2,
    title: "Viva Practical Exam Schedule B Sc Regular Semester 1 FAS Winter Examination 2025 26 08",
              ...
              {SESSIONS.map((session) => (
                <label key={session} className="flex items-center gap-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 text-[#0eb1df] focus:ring-[#0eb1df] border-gray-300 rounded"
                  />
                  <span className="text-gray-800 text-sm font-medium">{session}</span>
                </label>
              ))}
            </div>
          </div>
              ...
              {PROGRAMS.map((program) => (
                <label key={program} className="flex items-center gap-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 text-[#0eb1df] focus:ring-[#0eb1df] border-gray-300 rounded"
                  />
                  <span className="text-gray-800 text-sm font-medium">{program}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          {SCHEDULES.map((schedule) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-gray-900 font-bold text-sm md:text-base pr-4 leading-snug max-w-lg">
                  {schedule.title}
                </h3>
              </div>
              <Link
