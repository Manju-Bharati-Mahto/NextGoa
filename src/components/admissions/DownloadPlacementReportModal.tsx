...
export function DownloadPlacementReportModal() {
  const [isOpen, setIsOpen] = useState(false);
  const [submitState, setSubmitState] = useState<'idle' | 'submitting' | 'success'>('idle');
  const [formData, setFormData] = useState({ name: "", email: "", phone: "" });
  const [touched, setTouched] = useState<Record<string, boolean>>({});

  const [pdfUrl, setPdfUrl] = useState('/documents/placement_brochure_2025_2026.pdf');
  const [pdfFilename, setPdfFilename] = useState('Placement_Report_2025_2026.pdf');

  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      const target = (e.target as Element).closest("[data-placement-trigger]");
      if (target && target.getAttribute("data-placement-trigger") === "true") {
        e.preventDefault();
        ...
        if (customUrl) {
          ...
          setPdfUrl('/documents/placement_brochure_2025_2026.pdf');
          setPdfFilename('Placement_Report_2025_2026.pdf');
        }
          ...
          <div className="flex flex-col items-center justify-center py-16 px-8 space-y-6 text-center animate-in zoom-in-50 fade-in duration-500">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[15px] text-gray-500 leading-relaxed">
                Your placements report is downloading automatically. We hope you find it helpful!
              </p>
            </div>
          </div>
        ) : (
          <div className="p-6 sm:p-8 pt-8 sm:pt-10">
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-500 mt-2">
                Please enter your details to receive the comprehensive placements report.
              </p>
            </div>
