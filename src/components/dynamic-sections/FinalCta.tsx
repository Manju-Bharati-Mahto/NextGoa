...
export function FinalCta({ data }: FinalCtaProps) {
  ...
  const background = data?.background || "gradient";

  const backgroundImage = data?.backgroundImage || "/thankyou.png";
  return (
            ...
            {actions.length > 0 ? (
              <div className="mt-7 flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3">
                {actions.map((action) => {
                  ...
                  const commonProps = {
                    "data-enquiry-trigger": action.triggerEnquiry
                      ? "true"
                      : undefined,
                  ...
                  return isButton ? (
                    <button key={action.label} {...commonProps} type="button">
                      {action.icon === "phone" ? <PhoneIcon /> : null}
                      {action.label}
                    </button>
                  ) : (
                    <a key={action.label} {...commonProps} href={action.href}>
                      {action.icon === "phone" ? <PhoneIcon /> : null}
                      {action.label}
                    </a>
                  );
