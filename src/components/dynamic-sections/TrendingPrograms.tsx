...
export function TrendingPrograms({ data }: Props) {
  ...
  const ActiveIcon = ICONS[activeCourse.icon] || EngineeringIcon;
  

  return (
    <section
      className="relative z-20 w-full lg:min-h-[600px] flex items-center bg-transparent -mt-12 sm:-mt-24 lg:-mt-5 lg: py-16 sm:py-24"
      style={{
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-4 lg:gap-0 items-center">
          ...
          <div className="flex flex-col text-white lg:pr-4 relative z-10">
            <h2 className="section-subheading text-white"> {data?.eyebrow}</h2>
            <h3
              className="text-[#FFD523] mt-2 leading-[1.1] section-heading"
            ...
            <div className="mt-0 sm:mt-2 w-full max-w-[800px]">
              <div className="flex justify-end mb-3 pr-2">
                <a
                  ...
                  className="text-sm font-medium font-poppins text-white/90 hover:text-white flex items-center gap-1 transition-colors"
                >
              ...
              >
                ...
                <div
                  ...
                  className="absolute -right-4 -top-4 sm:-right-4 sm:-top-4 pointer-events-none transition-all duration-500 w-[140px] h-[140px] sm:w-[240px] sm:h-[240px] opacity-[0.4] flex items-center justify-end animate-fade-in"
                >
                  <ActiveIcon
                ...
                >
                  <div className="flex flex-wrap gap-2 sm:gap-3 mb-5">
                    {(activeCourse.tags || []).map(
                      (tag: string, idx: number) => (
                        <span
                          key={idx}
                          className="bg-sunshine text-[11px] font-bold px-3 py-1.5 rounded-full uppercase tracking-widest text-ink"
                        >
                          {tag}
                        </span>
                      ),
                  ...
                  <h4 className="text-3xl sm:text-[38px] font-semibold tracking-tight mb-6 font-[family-name:var(--font-poppins)]">
                    {activeCourse.title}
                  </h4>

                  <a
                    href={activeCourse.href}
                    className="bg-[#0EB1E1] hover:bg-[#0BA1CD] transition-colors text-white text-[14px] font-medium px-6 py-2.5 rounded-full flex items-center gap-2 w-max"
                  >
                    {activeCourse.buttonLabel || "Apply Now"}
                    <span>&rarr;</span>
                  </a>
                </div>
              </div>
          ...
          <div className="hidden lg:flex justify-end relative h-full lg:mt-0 lg:-ml-16 z-0 pointer-events-none">
            <Image
              src={data?.rightImage || "/programmes/trending-right.png"}
