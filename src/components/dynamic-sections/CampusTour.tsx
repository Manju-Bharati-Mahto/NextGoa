...
export function CampusTour({ data, }: CampusTourProps) {
  ...
  return (
    ...
    >
      ...
      >
        <Eyebrow className="text-white">{data?.eyebrow}</Eyebrow>
          ...
          {!isPlaying ? (
            ...
            >
              <div
                ...
                style={{ backgroundImage: `url(${ data?.thumbnail || "/campus-tour-thumbnail.webp" })`, }}
              >
                {/* Dark overlay for readability */}
                <div className="absolute inset-0 bg-black/35 transition-opacity group-hover:bg-black/45" />
                <div className="relative flex flex-col items-center z-10">
                  <PlayButton />
                  <p className="mt-4 text-lg font-bold text-white tracking-tight">
                   {data?.watchButtonText}
                  </p>
                  <p className="text-sm text-white/80 font-[family-name:var(--font-poppins)]">
                    {data?.locationText}
                  </p>
                </div>
              </div>
            ...
            <iframe
              width="100%"
              height="100%"
              src={`${ data?.videoUrl || "https://www.youtube.com/embed/epXIhN_Pt7I" }?autoplay=1`}
