...
export function CareerListingsTest() {
  ...
  // Listen to search events from the CareerHeroSearchTest component
  useEffect(() => {
    ...
    window.addEventListener("career-search-test", handleSearchEvent);
    return () => window.removeEventListener("career-search-test", handleSearchEvent);
  }, []);
  ...
  useEffect(() => {
    async function fetchJobs() {
      try {
        ...
        setError("Failed to fetch vacancies from database");
      } finally {
        setLoading(false);
      }
    }
        ...
        <div className="flex flex-col gap-6 lg:gap-8 h-fit">
          ...
          {loading ? (
            <div className="col-span-1 md:col-span-2 py-24 text-center bg-white rounded-3xl border border-gray-200 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-gray-500 font-medium">Fetching job openings from database...</p>
            </div>
          ) : error ? (
                ...
                jobs.map((job) => (
                  <div key={job.id} className="bg-white rounded-[2rem] border border-gray-200 p-6 shadow-sm flex flex-col">
                    ...
                    <div className="mt-auto flex items-center gap-4 w-full">
                      <Link href={`/careers-test/${job.slug}`} className="flex-1 bg-[#EF3341] hover:bg-[#D92A36] transition-colors text-white text-[16px] font-bold py-3 rounded-full text-center block">
                        Details
                      </Link>
