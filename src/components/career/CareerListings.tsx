                ...
                jobs.map((job) => (
                  <div key={job.id} className="bg-white rounded-[2rem] border border-gray-200 p-6 shadow-sm flex flex-col">
                    ...
                    <div className="mt-auto flex items-center gap-4 w-full">
                      <Link href={`/careers/${job.position.toLowerCase().replace(/\s+/g, '-')}/${job.slug}`} className="flex-1 bg-[#EF3341] hover:bg-[#D92A36] transition-colors text-white text-[16px] font-bold py-3 rounded-full text-center block">
                        Details
                      </Link>
