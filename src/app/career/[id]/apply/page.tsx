        ...
        <div className="absolute inset-0 z-10 flex items-center pointer-events-none">
          <div className="max-w-[1440px] w-full mx-auto px-6 lg:px-12 flex flex-col md:flex-row items-center h-full pointer-events-auto">
            ...
            <div className="flex-1 flex flex-col justify-center w-full max-w-[800px] md:pr-8">
              <Link href={`/career/${job.id}`} className="text-[#111111] hover:opacity-80 transition-opacity mb-6 font-medium text-sm w-fit flex items-center gap-2">
                &larr; Back to Job Details
              </Link>
              
              <h1 className="text-[#152347] font-extrabold text-[48px] md:text-[64px] lg:text-[84px] leading-[1.05] mb-8 tracking-tight">
                You are just one <br className="hidden lg:block"/>
                helicopter shot away <br className="hidden lg:block"/>
                from a winning career
              </h1>
              
              <button className="bg-[#EF3341] hover:bg-[#D92A36] transition-colors text-white text-[18px] font-bold px-10 py-3.5 rounded-full w-fit shadow-md">
                Submit now
              </button>
            </div>
          </div>
